describe('MoleculeTable', function () {
  const testMolecules = [{
    'entryId': 1,
    'name': 'DX9',
  ...
  beforeEach(function () {
    document.body.appendChild(ElementBuilder.generate('div')
      .setAttribute('id', 'molecule-table').element)
    document.body.appendChild(ElementBuilder.generate('div')
      .setAttribute('id', 'viewport').element)
  })

  afterEach(function () {
    document.getElementById('molecule-table').remove()
    document.getElementById('viewport').remove()
  })

  it('creates the molecule table', function () {
    const stage = new NGL.Stage('viewport')
    const dummyTorsionResultsTable = { setMolecule: () => {} }
    new MoleculeTable('molecule-table', stage, dummyTorsionResultsTable, testMolecules)
    const rows = document.querySelectorAll('tr.odd, tr.even')
    expect(rows.length).toEqual(testMolecules.length)
    const cells = rows[0].getElementsByTagName('td')
    expect(cells.length).toEqual(3)
    expect(cells[0].innerText).toEqual(testMolecules[0].entryId.toString())
    expect(cells[1].innerText).toEqual(testMolecules[0].name)
    expect(cells[2].innerText).toEqual('Tolerable')
  })

  it('sets the molecules', function () {
    const stage = new NGL.Stage('viewport')
    const dummyTorsionResultsTable = { setMolecule: () => {} }
    const moleculeTable = new MoleculeTable('molecule-table', stage, dummyTorsionResultsTable)
    moleculeTable.setMolecules(testMolecules)
    const rows = document.querySelectorAll('tr.odd, tr.even')
    expect(rows.length).toEqual(testMolecules.length)
    const cells = rows[0].getElementsByTagName('td')
    expect(cells.length).toEqual(3)
  ...
  it('gets the worst quality', function () {
    const quality = MoleculeTable.getWorstQuality(testMolecules[0].torsionResults)
    expect(quality).toEqual('Tolerable')
  })

  it('applies the clicked row', function () {
    const stage = new NGL.Stage('viewport')
    const dummyTorsionResultsTable = { setMolecule: () => {} }
    const moleculeTable = new MoleculeTable('molecule-table', stage, dummyTorsionResultsTable, testMolecules)
    const rows = document.querySelectorAll('tr.odd, tr.even')
    expect(rows[0]).toHaveClass('highlight')

    const idCell = rows[1].getElementsByTagName('td')[0]
    const event = { target: idCell }
    moleculeTable.handleTableClicked(event)
